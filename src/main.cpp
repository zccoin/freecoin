...
unsigned int nStakeTargetSpacing = 45; // DIFF: 45 blocks block spacing
int64 nChainStartTime = 1375801200;//Tue, 06 Aug 2013 15:00:00 GMT

int nCoinbaseMaturity = 68;// mining need 88 confirm 68+20
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 FEC if made of 50 FEC inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
int64 GetProofOfWorkReward(unsigned int nBits, int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight<=1920)//1st day
    {
        nSubsidy = nSubsidy;
    }
    //comment the following code for debug
    else if(nHeight<=3840)//2nd day
    {
        nSubsidy = nSubsidy*0.8705;
    }
    else if(nHeight<=5760)//3rd day
    {
        nSubsidy = nSubsidy*pow(0.8705,2);
    }
    else if(nHeight<=7680)//4th day
    {
        nSubsidy = nSubsidy *pow(0.8705,3);
    }
    else if(nHeight<=9600)//5th day
    {
        nSubsidy = nSubsidy * pow(0.8705,4);
    }
    else //5th day // from 6th day to the end, block reward is 44 coins and varies based on the diff
    {
        CBigNum bnSubsidyLimit=MAX_MINT_PROOF_OF_WORK/2;
        CBigNum bnTarget;
        ...
        nSubsidy = min(nSubsidy, MAX_MINT_PROOF_OF_WORK/2);
    }
    if (fDebug && GetBoolArg("-printcreation"))
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal freecoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in freecoin
    bool fStrictPayToScriptHash = true; // Always active in freecoin
    ...
    // ppcoin: fees are not collected by miners as in freecoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    */
    ...
    if(pIndex0->pprev)
    {
    if (vtx[0].GetValueOut() > (IsProofOfWork()? (GetProofOfWorkReward(nBits,pIndex0->nHeight,MIN_TX_FEE,pIndex0->pprev->GetBlockHash()) - vtx[0].GetMinFee() + MIN_TX_FEE) : 0))
        return DoS(50, error("CheckBlock() : coinbase reward exceeded %s > %s", 
                   FormatMoney(vtx[0].GetValueOut()).c_str(),
                   FormatMoney(IsProofOfWork()? GetProofOfWorkReward(nBits,pIndex0->nHeight,MIN_TX_FEE,pIndex0->pprev->GetBlockHash()) : 0).c_str()));
    }
    */
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "FreeCoin - Edward Snowden: The Price of Being a ‘Whistleblower’ - By Michelle Singletary";
        CTransaction txNew;
        ...
        block.nBits    = bnProofOfWorkLimit.GetCompact();//0x1e0fffff;//bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 12235911;

        //// debug print
        printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
        printf("block.hashMerkleRoot == %s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x883447a4dcf57acf6969405affdfd45438ff03a1b532deb029153dbb8d5272ff"));
        block.print();
...
string GetWarnings(string strFor)
{
    ...
    if (Checkpoints::IsSyncCheckpointTooOld(nStakeMaxAge*100) && !fTestNet && !IsInitialBlockDownload())
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old.";
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("FreecoinMiner : proof-of-work not meeting target");
...
void static ThreadFreecoinMiner(void* parg);

static bool fGenerateFreecoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void FreecoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateFreecoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload()|| (fProofOfStake && vNodes.size() < 3 && nBestHeight < GetNumBlocksOfPeers()))
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateFreecoins) && !fProofOfStake)
                return;
        }
...
void static ThreadFreecoinMiner(void* parg)
{
    ...
    try
    {
        ...
        FreecoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadFreecoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadFreecoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadFreecoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateFreecoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateFreecoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateFreecoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d FreecoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadFreecoinMiner, pwallet))
                printf("Error: NewThread(ThreadFreecoinMiner) failed\n");
            Sleep(10);
        }
    }
}
